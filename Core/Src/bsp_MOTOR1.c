...
/**
  ...
  */
/*
void MOTOR1_TIM2_IRQHandler(void) //定时器中断处理
{
  ...
  if (__HAL_TIM_GET_IT_SOURCE(&htim2_MOTOR1, MOTOR1_TIM2_IT_CCx) != RESET)
  {
    ...
    if (i % 2 == 0) //每进入两次中断才是一个完整脉冲
    {
      ...
      case ACCEL: //加速
        ...
        __HAL_TIM_SET_COMPARE(&htim2_MOTOR1, MOTOR1_TIM2_CHANNEL_x, period_MOTOR1[Motor1_num] / 2);
        Motor1_num++;
    ...
    if (i == 2) // 2次，说明已经输出一个完整脉冲
    {
      ...
      case ACCEL:
        ...
        if (step_count >= Motor1_srd.decel_start)                                                                           // 检查是够应该开始减速
        {
          ...
          Motor1_srd.run_state = DECEL;                  // 下个脉冲进入减速阶段
        }
        else if (new_step_delay <= Motor1_srd.min_delay) // 检查是否到达期望的最大速度
        {
          ...
          rest = 0;                              // 清零余值
          Motor1_srd.run_state = RUN;            // 设置为匀速运行状态
        }
        break;
      ...
      case RUN:
        ...
        if (step_count >= Motor1_srd.decel_start) // 需要开始减速
        {
          ...
          Motor1_srd.run_state = DECEL;                  // 状态改变为减速
        }
        break;
