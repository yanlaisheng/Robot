...
__IO uint32_t acc_speed_len_Motor5; //加减速长度
/* 扩展变量 ------------------------------------------------------------------*/
...
/**
  ...
  *           速度很慢，那还没达到最大速度就要开始减速
  */
void MOTOR5_AxisMoveRel_S(int32_t step, uint32_t speed, uint16_t Dir, uint16_t Acc_len)
{
  ...
  if (step > 2 * Acc_len)
  {
    acc_speed_len_Motor5 = Acc_len;
  }
  else if ((step > 4) && (step <= Acc_len))
  {
    acc_speed_len_Motor5 = step >> 1;
  }
  else if ((step >= 1) && (step < 4))
  {
    acc_speed_len_Motor5 = 0;
  }
  else if (step == 0)
    return;

  step_to_run_MOTOR5 = step - 2 * acc_speed_len_Motor5; //匀速步数

  fre_Set = speed * SPR / 60; //转换为运行频率
  // 如果最大速度很慢，<最低频率，则不需要进行加速运动
  if ((fre_Set > FRE_MIN_MOTOR5) && (acc_speed_len_Motor5 != 0))
  {
    CalculateSModelLine(period_MOTOR5, acc_speed_len_Motor5, acc_speed_len_Motor5 * fre_Set / ACCELERATED_SPEED_LENGTH, FRE_MIN_MOTOR5, 4);
    Motor5_status = ACCEL;
  }
  else
  {
    step_to_run_MOTOR5 = step; //全部为匀速运动
    Motor5_status = RUN;
  }
