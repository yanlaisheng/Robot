...
#define MOTOR5_TIM1_GPIO_CLK_ENABLE() __HAL_RCC_GPIOC_CLK_ENABLE() // 输出控制脉冲给电机驱动器
#define MOTOR5_TIM1_PUL_PORT GPIOE                                 // 对应驱动器的PUL-（驱动器使用共阳接法）
#define MOTOR5_TIM1_PUL_PIN GPIO_PIN_9                             // 而PLU+直接接开发板的VCC

#define MOTOR5_DIR_GPIO_CLK_ENABLE() __HAL_RCC_GPIOD_CLK_ENABLE() // 电机旋转方向控制，如果悬空不接默认正转
...
#define MOTOR5_ENA_GPIO_CLK_ENABLE() __HAL_RCC_GPIOD_CLK_ENABLE() // 电机脱机使能控制，如果悬空不接默认使能电机
