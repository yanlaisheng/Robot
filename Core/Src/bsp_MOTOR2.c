...
void MOTOR2_AxisMoveRel(int32_t step, uint32_t accel, uint32_t decel, uint32_t speed)
{
  ...
  if (step < 0) // 步数为负数
  {
    MOTOR2_srd.dir = CCW; // 逆时针方向旋转
    MOTOR2_DIR_REVERSAL();
    step = -step; // 获取步数绝对值
  }
  else
  {
    MOTOR2_srd.dir = CW; // 顺时针方向旋转
    MOTOR2_DIR_FORWARD();
  }

  if (step == 1) // 步数为1
  {
    MOTOR2_srd.accel_count = -1;  // 只移动一步
    MOTOR2_srd.run_state = DECEL; // 减速状态.
    MOTOR2_srd.step_delay = 1000; // 短延时
  }
  ...
  MOTOR2_MotionStatus = 1; // 电机为运动状态
  tim_count = __HAL_TIM_GET_COUNTER(&htim4_MOTOR2);
  __HAL_TIM_SET_COMPARE(&htim4_MOTOR2, MOTOR2_TIM4_CHANNEL_x, tim_count + MOTOR2_srd.step_delay); // 设置定时器比较值
...
extern float _MOTOR2[ACCELERATED_SPEED_LENGTH];                //数组存储加速过程中每一步的频率
...
/**
  ...
  * 说    明: 实现加减速过程
  */
// void MOTOR2_TIM4_IRQHandler(void) //定时器中断处理
// {
...
//     {
//       i = 0;                        // 清零定时器中断次数计数值
//       switch (MOTOR2_srd.run_state) // 加减速曲线阶段
//       {
//       case STOP:
...
//         {
//           MOTOR2_step_position++; // 绝对位置加1
//         }
//         else
//         {
//           MOTOR2_step_position--; // 绝对位置减1
//         }
//         MOTOR2_srd.accel_count++;                                                                                           // 加速计数值加1
//         new_step_delay = MOTOR2_srd.step_delay - (((2 * MOTOR2_srd.step_delay) + rest) / (4 * MOTOR2_srd.accel_count + 1)); //计算新(下)一步脉冲周期(时间间隔)
//         rest = ((2 * MOTOR2_srd.step_delay) + rest) % (4 * MOTOR2_srd.accel_count + 1);                                     // 计算余数，下次计算补上余数，减少误差
//         if (step_count >= MOTOR2_srd.decel_start)                                                                           // 检查是够应该开始减速
//         {
//           MOTOR2_srd.accel_count = MOTOR2_srd.decel_val; // 加速计数值为减速阶段计数值的初始值
//           MOTOR2_srd.run_state = DECEL;                  // 下个脉冲进入减速阶段
//         }
//         else if (new_step_delay <= MOTOR2_srd.min_delay) // 检查是否到达期望的最大速度
//         {
//           last_accel_delay = new_step_delay;     // 保存加速过程中最后一次延时（脉冲周期）
//           new_step_delay = MOTOR2_srd.min_delay; // 使用min_delay（对应最大速度speed）
//           rest = 0;                              // 清零余值
//           MOTOR2_srd.run_state = RUN;            // 设置为匀速运行状态
//         }
//         break;
...
//         }
//         new_step_delay = MOTOR2_srd.min_delay;    // 使用min_delay（对应最大速度speed）
//         if (step_count >= MOTOR2_srd.decel_start) // 需要开始减速
//         {
//           MOTOR2_srd.accel_count = MOTOR2_srd.decel_val; // 减速步数做为加速计数值
//           new_step_delay = last_accel_delay;             // 加阶段最后的延时做为减速阶段的起始延时(脉冲周期)
//           MOTOR2_srd.run_state = DECEL;                  // 状态改变为减速
//         }
//         break;
...
//         break;
//       }
//       MOTOR2_srd.step_delay = new_step_delay; // 为下个(新的)延时(脉冲周期)赋值
//     }
// #endif
//   }
// }
