...
__IO uint32_t acc_speed_len_Motor6; //加减速长度
/* 扩展变量 ------------------------------------------------------------------*/
...
void MOTOR6_AxisMoveRel(int32_t step, uint32_t accel, uint32_t decel, uint32_t speed)
{
  ...
  if (Motor6_MotionStatus != STOP) // 只允许步进电机在停止的时候才继续
    return;
  if (step < 0) // 步数为负数
  {
    MOTOR6_srd.dir = CCW; // 逆时针方向旋转
    MOTOR6_DIR_REVERSAL();
    step = -step; // 获取步数绝对值
  }
  else
  {
    MOTOR6_srd.dir = CW; // 顺时针方向旋转
    MOTOR6_DIR_FORWARD();
  }

  if (step == 1) // 步数为1
  {
    MOTOR6_srd.accel_count = -1;  // 只移动一步
    MOTOR6_srd.run_state = DECEL; // 减速状态.
    MOTOR6_srd.step_delay = 1000; // 短延时
  }
  ...
  Motor6_MotionStatus = 1; // 电机为运动状态
  tim_count = __HAL_TIM_GET_COUNTER(&htim5_MOTOR6);
  __HAL_TIM_SET_COMPARE(&htim5_MOTOR6, MOTOR6_TIM5_CHANNEL_x, tim_count + MOTOR6_srd.step_delay); // 设置定时器比较值
...
/**
  ...
  *           速度很慢，那还没达到最大速度就要开始减速
  */
void MOTOR6_AxisMoveRel_S(int32_t step, uint32_t speed, uint16_t Dir, uint16_t Acc_len)
{
  ...
  if (step > 2 * Acc_len)
  {
    acc_speed_len_Motor6 = Acc_len;
  }
  else if ((step > 4) && (step <= Acc_len))
  {
    acc_speed_len_Motor6 = step >> 1;
  }
  else if ((step >= 1) && (step < 4))
  {
    acc_speed_len_Motor6 = 0;
  }
  else if (step == 0)
    return;

  step_to_run_MOTOR6 = step - 2 * acc_speed_len_Motor6; //匀速步数

  fre_Set = speed * SPR / 60; //转换为运行频率
  // 如果最大速度很慢，<最低频率，则不需要进行加速运动
  if ((fre_Set > FRE_MIN_MOTOR6) && (acc_speed_len_Motor6 != 0))
  {
    CalculateSModelLine(period_MOTOR6, acc_speed_len_Motor6, acc_speed_len_Motor6 * fre_Set / ACCELERATED_SPEED_LENGTH, FRE_MIN_MOTOR6, 4);
    Motor6_status = ACCEL;
  }
  else
  {
    step_to_run_MOTOR6 = step; //全部为匀速运动
    Motor6_status = RUN;
  }
...
// extern float fre_MOTOR6[ACCELERATED_SPEED_LENGTH]; //数组存储加速过程中每一步的频率
...
/**
  ...
  * 说    明: 实现加减速过程
  */
void MOTOR6_TIM5_IRQHandler(void) //定时器中断处理
{
  ...
  if (__HAL_TIM_GET_IT_SOURCE(&htim5_MOTOR6, MOTOR6_TIM5_IT_CCx) != RESET)
  {
    ...
    {
      i = 0;                        // 清零定时器中断次数计数值
      switch (MOTOR6_srd.run_state) // 加减速曲线阶段
      {
      case STOP:
      ...
      case ACCEL:
        ...
        {
          MOTOR6_step_position++; // 绝对位置加1
        }
        else
        {
          MOTOR6_step_position--; // 绝对位置减1
        }
        MOTOR6_srd.accel_count++;                                                                                           // 加速计数值加1
        new_step_delay = MOTOR6_srd.step_delay - (((2 * MOTOR6_srd.step_delay) + rest) / (4 * MOTOR6_srd.accel_count + 1)); //计算新(下)一步脉冲周期(时间间隔)
        rest = ((2 * MOTOR6_srd.step_delay) + rest) % (4 * MOTOR6_srd.accel_count + 1);                                     // 计算余数，下次计算补上余数，减少误差
        if (step_count >= MOTOR6_srd.decel_start)                                                                           // 检查是够应该开始减速
        {
          MOTOR6_srd.accel_count = MOTOR6_srd.decel_val; // 加速计数值为减速阶段计数值的初始值
          MOTOR6_srd.run_state = DECEL;                  // 下个脉冲进入减速阶段
        }
        else if (new_step_delay <= MOTOR6_srd.min_delay) // 检查是否到达期望的最大速度
        {
          last_accel_delay = new_step_delay;     // 保存加速过程中最后一次延时（脉冲周期）
          new_step_delay = MOTOR6_srd.min_delay; // 使用min_delay（对应最大速度speed）
          rest = 0;                              // 清零余值
          MOTOR6_srd.run_state = RUN;            // 设置为匀速运行状态
        }
        break;
      ...
      case RUN:
        ...
        new_step_delay = MOTOR6_srd.min_delay;    // 使用min_delay（对应最大速度speed）
        if (step_count >= MOTOR6_srd.decel_start) // 需要开始减速
        {
          MOTOR6_srd.accel_count = MOTOR6_srd.decel_val; // 减速步数做为加速计数值
          new_step_delay = last_accel_delay;             // 加阶段最后的延时做为减速阶段的起始延时(脉冲周期)
          MOTOR6_srd.run_state = DECEL;                  // 状态改变为减速
        }
        break;
      ...
      MOTOR6_srd.step_delay = new_step_delay; // 为下个(新的)延时(脉冲周期)赋值
    }
#endif
  }
}
