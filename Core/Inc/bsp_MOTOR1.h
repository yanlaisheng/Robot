...
#define MOTOR1_TIM2_GPIO_CLK_ENABLE() __HAL_RCC_GPIOC_CLK_ENABLE() // 输出控制脉冲给电机驱动器
#define MOTOR1_TIM2_PUL_PORT GPIOA                                 // 对应驱动器的PUL-（驱动器使用共阳接法）
#define MOTOR1_TIM2_PUL_PIN GPIO_PIN_1                             // 而PLU+直接接开发板的VCC

#define MOTOR1_DIR_GPIO_CLK_ENABLE() __HAL_RCC_GPIOD_CLK_ENABLE() // 电机旋转方向控制，如果悬空不接默认正转
...
#define MOTOR1_ENA_GPIO_CLK_ENABLE() __HAL_RCC_GPIOD_CLK_ENABLE() // 电机脱机使能控制，如果悬空不接默认使能电机
